...
final_result_init |> 
  ggplot(aes(x = factor(Cluster_Num, levels = c(1, 2, 4, 7, 8, 9), ordered = TRUE), y = One)) +
  geom_boxplot() +
...
final_result_init |> 
  ggplot(aes(x = factor(Cluster_Num, levels = c(1, 2, 4, 7, 8, 9), ordered = TRUE), y = Neg_One)) +
  geom_boxplot() +
  ggtitle("15 Clusters, 30 n, with 20 resamples on a random 3d UNIFORM
          (Generates New Simulated Data per n)") +
  scale_y_continuous(limits = c(0, 5000)) +
...
kmeans <- k_means(num_clusters = 4) |>
 fit(~ X1 + X2,
     data = random_sample)
data |> 
  plot_ly(x = ~X1, y = ~X2, z = ~X3, color = )
```
